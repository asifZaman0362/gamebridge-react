...
/**
 * A minimal push/pop scene stack — Pixi has no scene manager of its own, unlike
 * Phaser. Each pushed scene's view sits above the ones beneath it on the shared
 * stage, and its `update` runs every tick while it's on top.
 *
 * This demo only ever calls {@link SceneManager.start}, which replaces the
 * whole stack — mirroring the Phaser demo's `game.scene.start(key)` — but it's
