...
function StartGame(parent: string) {
  ...
  // Boot first, then attach: a `LoadScene` queued before the engine existed is
  // drained the moment its handler attaches, and must land after Boot has
  // handed over to the main menu — not be replaced by it.
  manager.start("Boot");
